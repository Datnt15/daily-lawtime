...
<div id="blog-wrapper" class="free_text_search_result">
	...
	<?php 
	$nonce_name   = isset( $_POST['active_code'] ) ? $_POST['active_code'] : '';
	if ( wp_verify_nonce( $nonce_name, 'resend_active_code' ) ) {
		$user_meta = get_user_meta(get_current_user_id());
        $message = "Hi, {$user_meta['nickname'][0]}. ";
    	$message .= 'Please click on this link below to activate your account: ';
		$message .= get_site_url() . '/activate?uid=' . get_current_user_id() . '&secret_code=' . $user_meta['secret-code'][0];
		$url = "http://bulk.sms-india.in/unified.php?usr=28790&pwd=udaipur123&ph=".$user_meta['phone_number'][0]."&sndr=DAILYL&text=$message";
  		@file_get_contents($url);
    }
	if (is_user_logged_in() && isset($_GET['file']) && isset($_GET['type'])) :
		...
		if (is_user_active()):
			...
			<h3 class="text-center">You need to active your account to see this document</h3>
			<form action="" method="POST" class="text-center">
				<?php wp_nonce_field( 'resend_active_code', 'active_code' ); ?>
				<input type="submit" class="button-primary" value="Resend Activation Code">
			</form>
		<?php endif; ?>
